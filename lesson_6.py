...
# Anna – correct. You could also use i.isalpha() or i.isdigit() methods to check if i is a letter or a digit.
...
# For example, if I input three numbers "10 20 30", your code will treat it as "102030" (a 6-digit string). 
# Also, you convert the result to an integer after calculating the median, which changes the correct value. So it would be better, if you want to get a list as input, to ask users to enter digits separated by a delimiter (e.g., comma), then split the string, convert each element to float/int, and only after that calculate the median.